      ...
      <div v-else class="p-fluid grid">
        <div class="field col-12">
          <p>
            ...
            {{
              maxUploads - (uploadedFiles.length + selected.attachments.length)
            }}
...
const { selected, items, loading, error, submitted } = storeToRefs(
...
const upload = async (event) => {
  try {
    for (const file of event.files) {
      const response = await nominationsStore.uploadAttachment(
        file,
        props.data.label,
...
const remove = async (data) => {
  ...
  uploadedFiles.value = uploadedFiles.value.filter(
    (file) => file.hasOwnProperty("_id") && file._id !== _id
  );
};
</script>
