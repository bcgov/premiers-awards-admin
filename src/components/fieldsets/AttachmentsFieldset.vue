    ...
    <template #message="slotProps">
      <div class="card p-4">
        <div class="flex mb-5">
          ...
          <span class="pl-2">Delete Attachment</span>
        </div>
        <div class="pl-2 w-80">
          <div class="grid">
    ...
    <div class="card">
      <div class="p-fluid grid">
        <div class="field col-12">
          <!-- Updated text (PA-159) -->
          <p>
            Please submit no more than <b>5</b> attachments (or 1 attachment of
            5 pages maximum) in support of the nomination. The combined total
            page count of your documents must not exceed <b>5</b> pages.
          </p>
          ...
          <template v-if="isIndividualNomination">
            <p>
              <b
                >Reminder: All individual nominations require 2 testimonials or
                letters of support.</b
              >
            </p>
          </template>
          <template v-else>
          ...
          <ul>
            <li>
              Any pages beyond the first 5 are automatically deleted from your
              nomination and will not be reviewed by the judges
            </li>
            <li>
              Attachments cannot include videos or hyperlinks. These will not be
              reviewed by the judges
            </li>
            <li>
              Be sure your attachments do not have access restrictions prior to
              submitting. Password protected attachments will be automatically
              deleted from your nomination
            </li>
            ...
            <template #header>
              <div class="grid">
                <div class="md:col-9 col-12">
                  <p>
                    <strong
                      >Only Adobe PDF (Portable Document Format) documents are
                      accepted.</strong
                    >
                  </p>
                </div>
                <div class="md:col-3 col-12">
                  <Button
                    :disabled="
                      selected.attachments.length >= 5 || selected.submitted
                    "
                    label="Attach File"
                    icon="pi pi-plus"
                    @click="add"
            ...
            <Column field="file" header="File" :sortable="true">
              <template #body="{ data }">
                <span
                  ...
                  v-tooltip.top="data.file.originalname"
                >
                  {{
                    !data.file
                      ? "(No File)"
...
const { selected, items, loading, downloading, error, submitted } = storeToRefs(
...
const remove = (data) => {
  ...
  confirm.require({
    ...
    message: {
      ...
      filename:
        file && file.hasOwnProperty("originalname")
          ? file.originalname
          : "(No file)",
    },
